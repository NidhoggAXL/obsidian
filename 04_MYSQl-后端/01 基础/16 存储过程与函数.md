...
1. [](04_MYSQl-后端/01%20基础/16%20存储过程与函数.md#2.2.1%20不带参数符号|没有参数（无参数无返回）)
2. [](04_MYSQl-后端/01%20基础/16%20存储过程与函数.md#2.2.3%20IN|仅仅带%20IN%20类型（有参数无返回）)
3. [](04_MYSQl-后端/01%20基础/16%20存储过程与函数.md#2.2.2%20OUT|仅仅带%20OUT%20类型（无参数有返%20回）)
4. [](04_MYSQl-后端/01%20基础/16%20存储过程与函数.md#2.2.4%20IN%20和%20OUT%20联合使用|既带%20IN%20又带%20OUT（有参数有返回）)
5. [](04_MYSQl-后端/01%20基础/16%20存储过程与函数.md#2.2.5%20带%20INOUT|带%20INOUT（有参数有返回）)
...
> * 输入这个参数的时候要注意，函数里面使用这个参数时候的类型，**只可以大不可以小**，尽量保持类型一致
...
> 3. SET：赋值语句，用于对变量进行赋值。 4. SELECT… INTO：把从数据表中查询的结果存放到变量中，也就是为变量赋值。
...
```mysql
delimiter //
CREATE PROCEDURE shou_min_salary(OUT ms DOUBLE)
BEGIN 
...
### 2.2.3 IN
举例5：创建存储过程show_someone_salary()，查看“employees”表的某个员工的薪资，并用IN参数empname 输入员工姓名。
```mysql
delimiter //
...
#调用2：赋值一个参数，在传入函数
SET @empname = 'Abel';# = 也可以写成明确的赋值符号 (:=)
CALL show_someone_salary2(@empname);
# 这里 @ 后面的名字可以是自定义的名字，不一定要和参数命相同
```

### 2.2.4 IN 和 OUT 联合使用
举例6：创建存储过程show_someone_salary2()，查看“emplyees”表的某个员工的薪资，并用IN参数empname 输入员工姓名，用OUT参数empsalary输出员工薪资。
```mysql
delimiter //
...
#调用
SET @empname = 'Abel';
CALL show_some_salary3 (@empname , @empsalary);
#查看
SHOW @empsalary;
```
...
## 3.2 SQLyog 和 Navicat 关于 DELIMITER 区别
...
## 4.5 对比存储函数和存储过程
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1713968236000tq29os.png)
...
1. 使用SHOW CREATE语句查看存储过程和函数的**创建信息**
基本语法结构如下：
```mysql
...
```
2. 使用SHOW STATUS语句查看存储过程和函数的**状态信息**
基本语法结构如下：
```mysql
...
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/171411304700020sz3u.png)
