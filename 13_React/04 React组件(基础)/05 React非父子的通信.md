...
- 如果我们在顶层的App中定义这些信息，之后一层层传递下去，那么**对于一些中间层不需要数据的组件来说，是一种冗余的 操作**。
...
- 挂载在 **class(类)** 上的 contextType 属性会被重赋值为一个由React.createContext() 创建的 Context 对象： 
...
> [!info] 子组件没有作为 UserContext.Provider 的 children 的时候，不是孩子的时候
