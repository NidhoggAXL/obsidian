...
- 如果每次调用 setState 都进行一次更新，那么意味着render函数会被频繁调用（render 渲染阶段是可以被打断的），界面重新渲染，这样效率是很低的； 
...
> [!tip] 
> 
> 这些同步代码的本质就是，本来该 **React 进行回调的函数**，通过编写**交给了浏览器来处理**
...
> [!tip] 批处理自我理解：
> 
> 和[节流](07_JavaScript高级/17%20防抖、节流、深拷贝%20事件总线/01%20认识防抖和节流.md)一样，在规定时间内执行一次，不管事件的多少都是在一定时间内统一执行
