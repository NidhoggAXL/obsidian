...
- class组件有自己的生命周期，可以在对应的生命周期中完成自己的逻辑
	- 比如在 [](13_React/04%20React组件-基础/02%20React组件生命周期.md#2.3%20常用生命周期|componentDidMount) 中发送网络请求，并且该生命周期函数只会执行一次;
...
- 比如 [](13_React/04%20React组件-基础/02%20React组件生命周期.md#2.3%20常用生命周期|componentDidMount) 中，可能就会包含大量的逻辑代码：包括**网络请求、一些事件的监听**（还需要在 componentWillUnmount 中移除）； 
- 而对于这样的class实际上非常难以拆分：因为它们的**逻辑往往混在一起**，强行拆分反**而会造成过度设计，增加代码的复杂度**；

**难以理解的class：** 

- 很多人发现学习ES6的class是学习React的一个障碍。 
- 比如在class中，我们必须搞清楚 [this的指向](07_JavaScript高级/01%20函数this指向/01%20this的绑定规则.md) 到底是谁，所以需要花很多的精力去学习this； 
- 虽然前端开发人员必须掌握this，但是依然处理起来非常麻烦；

**组件复用状态很难：** 

- 在前面为了一些状态的复用我们需要通过[高阶组件](13_React/05%20React组件(进阶)/04%20React的高阶组件.md)；
- 像我们之前学习的**redux中 [connect](13_React/08%20Redux的使用/09%20connect原理.md) 或者react-router中的 [](13_React/10%20React-Router/04%20Router的代码跳转.md#类组件)**，这些高阶组件设计的目的就是为了**状态的复用**； 
- 或者类似于Provider、Consumer来共享一些状态，但是多次使用 [Consumer](13_React/04%20React组件-基础/05%20React非父子的通信.md) 时，我们的代码就会存在很多嵌套； 
...
- useState来自react，需要从react中导入，它是一个hook； 
	...
	- 返回值：数组，包含两个元素([](15_TypeScript/02%20TS基础语法/03%20TS数据类型.md#六、tuple类型|元组类型))； 
