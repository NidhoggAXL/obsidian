...
- 用法一：引入DOM元素
	- 在React中还有一种方式在类组件中获取到DOM，使用 [[02 获取DOM方式refs#一、如何使用ref|createRef]]； 
- 用法二：保存一个数据，这个对象在整个生命周期中可以保存不变；

```jsx
cont refContainer = useRef(initialValue)
```
...
useLayoutEffect看起来和 [[02 useState和useEffect(核心)#二、认识Effect Hook|useEffect]] 非常的相似，事实上他们也只有一点区别而已：
