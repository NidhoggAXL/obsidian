...
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1714200960000f9n9kh.png)
...
* 方式2：在MySQL服务运行期间，使用“set”命令重新设置系统变量的值
	* **针对当前会话是有效的，一旦结束会话，重新建立起新的会话，，就回复默认值**
```mysql
#为某个系统变量赋值 
#方式1： 
SET @@global.变量名=变量值; 
...
### 1.2.3 局部变量
定义：可以使用 `DECLARE-declare（声明）` 语句定义一个局部变量
...
BEGIN
	DECLARE emp_sal DOUBLE DEFAULT(0.0);#员工工资
	DECLARE mgr_sal DOUBLE DEFAULT(0.0);#员工工资
	
	DECLARE mgr_id INT DEFAULT(0);#管理者id
	
	SELECT salary INTO emp_sal FROM employees WHERE employee_id = emp_id;#查询员工的工资
	SELECT manager_id INTO mgr_id FROM employees WHERE employee_id = emp_id;#查询员工的管理者
	SELECT salary INTO mgr_sal FROM employees WHERE employee_id = mgr_id;#查询管理者的工资
...
### 1.2.4 对比会话用户变量与局部变量
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1714208337000bfw8cu.png)
...
调用存储过程：
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1714289655000h2borr.png)
...
调用：
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/171429080200058yjoo.png)
...
AS S
ELECT * FROM atguigudb.`departments`; 
...
BEGIN 
	...
	SET @x = 2; 
	INSERT INTO departments(department_name) VALUES('测试'); S
	ET @x = 3;
END // 
DELIMITER ;
```
...
调用存储过程：
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1714291499000vr0ago.png)
...
BEGIN
	...
	DECLARE hire_year DOUBLE;#记录入职时间
	...
	#查询入职时间
	SELECT DATEDIFF(CURDATE(),hire_data)/365 INTO hire_year
	FROM employees
	WHERE employee_id = emp_id;
...
#查找工资符合条件，方便后面调用观察存储过程是否执行正确
SELECT DATEDIFF(CURDATE(),hire_data)/365,employee_id,salary
FROM employees
WHERE DATEDIFF(CURDATE(),hire_data)/365 > 5 AND salary < 8000;
...
BEGIN 
	DECLARE emp_sal DOUBLE; 
	DECLARE hire_year INT; # 截断了小数，使用 INT 类型
	
	SELECT salary INTO emp_sal 
	FROM employees 
	WHERE employee_id = emp_id; 
	
	SELECT ROUND(DATEDIFF(CURDATE(),hire_date)/365) INTO hire_year # ROUND 截断小数，后面round(整数，小数)，小数 0 省略写了
	FROM employees 
	WHERE employee_id = emp_id; 
...
**举例1：**
使用LOOP语句进行循环操作，num值小于10时将重复执行循环过程。
```mysql
...
调用结果：
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1714307731000cjo8kp.png)

> 这里通过查询了十次，可以看出循环了10次。**同时也表示在循环的过程中是可以进行查询的，并不是要到循环结束才可以进行查询**。
...
BEGIN
	...
	lable_loop:LOOP
		#优先确定原本的平均工资是否已经大于12000
		IF avg_sal >= 12000 THEN LEAVE lable_loop;#leave 离开
		END IF;#对于if判断
		...
		#应该涨过工资，工资发生变化，需要重修确定平均工资
		SELECT AVG(salary) INTO avg_sal FROM employees ;
	END LOOP lable_loop;#对出循环
END;
...
为什么会错误，这个后面 `selec @num`的结果是null ，这是为什么呢？

> [!tip] 注解（关键）：
> 这里在循环的结构里面 num 是在里面运算了，**当退出循环的时候 num 并不是为 null 的 （特别注意）**，那为什么 num 在返回（OUT）的时候结果会是 null ，这个 null 的本质是没有返回任何数值。
> > **借助 C 语言的理解就是，循环里面的临时变量（形参）的数值并不能返传递实参（num）。**
...
BEGIN
	...
	lable_loop:LOOP
		#优先确定原本的平均工资是否已经大于12000
		IF avg_sal >= 22000 THEN LEAVE lable_loop;#leave 离开
		END IF;#对于if判断
...
> [!tip] 正确代码注解：
> 临时变量（形参）后面并没有传递给实参，而是通过创建一个临时变量`DECLARE loop_count INT DEFAULT(0);#记录循环次数`直接把临时变量的值赋值给实参`SET num = loop_count;`，并没有传递。
...
## 3.5 循环结构之 REPEAT
REPEAT语句创建一个带条件判断的循环过程。与WHILE循环不同的是，REPEAT 循环**首先会执行一次循环**，然后在 UNTIL 中进行表达式的判断，如果满足条件就退出，即 END REPEAT；如果条件不满足，则会 就继续执行循环，直到满足退出条件为止。
...
> **注意事项**：`until 结束循环条件表达式`写完后面不要加`;`号。
...
BEGIN 
	DECLARE avg_sal DOUBLE ; 
	ECLARE repeat_count INT DEFAULT 0; 
...
2. LOOP：一般用于实现简单的"死"循环 （借助 IF 语句来进行死循环）
3. WHILE：先判断后执行 
...
BEGIN 
	...
	while_label:WHILE TRUE DO  #② 循环条件
		 ...
		 #④ 迭代条件 S
		 ELECT AVG(salary) INTO avg_sal FROM employees; 
	 END WHILE; 
...
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/1714458061000hv9v93.png)
...
BEGIN
	DECLARE emp_sal DOUBLE DEFAULT(0.0);#某一个员工的工资
	DECLARE sum_sal DOUBLE DEFAULT(0.0);#工资的集合
	DECLARE emp_count INT DEFAULT(0);
	
	#游标的声明
	DECLARE emp_sure CURSOR FOR SELECT salary FROM employees ORDER BY salary DESC;
...
CALL get_count_by_limit_total_salary(2000000,@emp_count);
SELECT @emp_count;
```
...
![gh](https://raw.githubusercontent.com/AXLflechazoPN/Obsidian/main/2024/17144601740001woess.png)
