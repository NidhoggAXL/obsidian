...
* static修饰函数该表了函数的链接属性（普通的一个函数具有**外部链接属性**可以使用[[声明外部符号|extern(声明外部符号|[声明外部符号|[声明外部符号|[声明外部符号|[声明外部符号|[声明外部符号|[声明外部符号|[声明外部符号)]]]]]]]]).md)在本源代码文件中使用），使用了static修饰函数后就变为了**内部链接属性**，只可以在本源代码文件中使用。
