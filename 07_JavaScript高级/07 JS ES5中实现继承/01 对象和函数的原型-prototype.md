# 一、认识对象原型
**JavaScript当中==每个对象都有==一个特殊的内置属性 `[prototype](prototype)`（隐式原型），这个特殊的对象可以指向==另外一个对象==。**

那么这个对象有什么用呢?

* 当我们通过引用对象的属性key来获取一个value时，它会触发`[Get](Get)`的操作;
* 这个操作会**首先检查该对象是否有对应的属性，如果有的话就使用它**
* 如果对象中没有改属性，那么**会访问对象`[prototype](prototype)`内置属性指向的对象上的属性**
	* `[prototype](prototype)`原型里面也还有很多的对象
