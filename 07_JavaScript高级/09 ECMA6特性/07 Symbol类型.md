...
* 比如开发中我们使用[[07_JavaScript高级/08 JS ES6中实现继承/02 extends实现继承#四、类的混入mixin（了解）|混入]]，那么混入中出现了同名的属性，必然有一个会被覆盖掉;

Symbol就是为了解决上面的问题，用来生成**一个独一无二的值**。

* Symbol值是通过**Symbol函数**来生成的，生成后可以[[07_JavaScript高级/08 JS ES6中实现继承/06 ES6对象的增强|作为属性名]]
