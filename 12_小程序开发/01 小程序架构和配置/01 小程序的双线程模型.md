...
当小程序基于 WebView环境下时，WebView的 [[07_JavaScript高级/02 深入浏览器运行逻辑/02 浏览器渲染流程#二、渲染引擎解析页面|JS 逻辑、DOM树创建、CSS解析、样式计算、Layout、Paint(composite]])都发生在同一线程，在 WebView 上执行过多的 JS 逻辑可能阻塞渲染，导致界面卡顿。
