...
首相获取 state ，可以通过 [](02%20Redux的使用.md#二、Redux的使用过程|store.getState()) 来获取，那么 state 的类型应该就是 store.getState() 的放回值类型，获取返回值类型可以使用 [](06%20类型工具和类型体操.md#2.10%20ReturnType`<Type>`|ReturnType`<Type>`) 来获取。
