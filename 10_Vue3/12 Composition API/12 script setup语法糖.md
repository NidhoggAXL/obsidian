...
`<script setup>` 是在[[SFC)](10_Vue3/06 Vue基础-VCLI脚手架/01 Vue的开发模式解析.md|单文件组件 (SFC)]] 中使用组合式 Composition API 的编译时语法糖，当同时使用 SFC 与组合式 API 时则推荐该语法。
...
* `<script setup>`中的代码会在<mark class="hltr-cyan">每次组件实例被创建的时候执行</mark>。
...
使用 `<script setup>` 的组件时是**默认关闭**的

* 通过模板 [[10_Vue3/12 Composition API/08 setup中使用ref|ref ]] 或者 [](10_Vue3/11%20组件化额外知识/02%20组件中的ref引用.md#二、`# 一、script setup语法
...
* 通过模板 [[10_Vue3/12 Composition API/08 setup中使用ref|ref ]] 或者 parent和`# 一、script setup语法
...
* 通过模板 [[10_Vue3/12 Composition API/08 setup中使用ref|ref ]] 或者 root|$parent) 链获取到组件的公开实例，不会暴露任何在 `<script setup>` 中声明的绑定
...
* 父组件里面来<mark class="hltr-cyan">使用子组件实例的foo方法</mark>
