...
`<script setup>` 是在[单文件组件 (SFC)](SFC))) 中使用组合式 Composition API 的编译时语法糖，当同时使用 SFC 与组合式 API 时则推荐该语法。
...
* 通过模板 [ref ](10_Vue3/12%20Composition%20API/08%20setup中使用ref.md) 或者 [](10_Vue3/11%20组件化额外知识/02%20组件中的ref引用.md#二、`# 一、script setup语法
...
* 通过模板 [ref ](10_Vue3/12%20Composition%20API/08%20setup中使用ref.md) 或者 parent和`# 一、script setup语法
...
* 通过模板 [ref ](10_Vue3/12%20Composition%20API/08%20setup中使用ref.md) 或者 root|$parent) 链获取到组件的公开实例，不会暴露任何在 `<script setup>` 中声明的绑定
