...
* **v-model指令**可以在表单 [[05_前端基础/01 HTML + CSS/05 高级元素/01 高级元素的使用#5.1 input|input]]、[[05_前端基础/01 HTML + CSS/05 高级元素/01 高级元素的使用#5.7 textarea 的使用|textarea]] 以及 [[05_前端基础/01 HTML + CSS/05 高级元素/01 高级元素的使用#5.8 select 和 option 的使用|select]] 元素上创建**双向数据绑定**； 
* 它会根据**控件类型**自动选取正确的方法来更新元素； 
* 尽管有些神奇，**但 v-model 本质上不过是语法糖**，它**负责监听用户的输入事件来更新数据**，并在某种极端场景下进行一些特殊处理；

使用到的技术：[[01 高级元素的使用#五、表单元素#五、表单元素#五、表单元素#五、表单元素#5.4 input 和 label 的关系|label]] 和 [[05_前端基础/01 HTML + CSS/08 HTML5/03 input元素的扩展内容#^1fb847|placeholder]]
