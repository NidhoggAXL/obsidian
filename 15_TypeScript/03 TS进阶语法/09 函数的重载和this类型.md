...
- 方案一：使用联合类型来实现； 
- 方案二：实现函数重载来实现；
- 方案三：使用[[15_TypeScript/02 TS基础语法/02 JS数据类型#六、Object类型|对象类型]]来实现
...
上面的代码默认情况下是可以正常运行的，也就是TypeScript在编译时，认为我们的this是可以正确去使用的： 这是因为**在没有指定this的情况，this会更具上下文[[15_TypeScript/02 TS基础语法/01 TS变量#三、变量的类型推导(推断|01 TS变量]]|推导)出来this的类型**；
