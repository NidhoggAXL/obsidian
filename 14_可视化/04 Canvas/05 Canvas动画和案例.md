...
Canvas绘图都是通过 JavaScript 去操控的，如要实现一些交互性动画是相当容易的。那Canvas是如何做一些基本动画的？ 
...
- setTimeout定时器不是非常精准的，因为setTimeout的回调函数是放到了[[07_JavaScript高级/14 事件循环/03 宏任务、微任务队列|宏任务]]中等待执行。 
...
1. **同步刷新**：它将回调函数的执行时机锁定在浏览器的每一次渲染之前，确保每秒执行次数与屏幕刷新率（**通常**是 60Hz，即 16.7ms/次）一致，无比流畅。
	- 实际调用次数取决于：
	    - 浏览器的刷新率
	    - JavaScript执行引擎的性能
	    - 系统负载等因素
