...
6. 以「> 」(小于再加一个空白)在每行开头以形成区块引言。
7. 以「>> 」(两个小于再加一个空白)在每行开头以形成第二阶层的区块引言。
8. 区块里能再使用部份Markdown标记，具体能使用的标记因应用系统的实作而有不同。
...
9. 以「数字. 」(数字、点、空白)或「数字) 」开头
10. 数字不必照顺序排列
11. 起始数字以列表第一个项目的数字开始递增
12. 列表项目间有空行时，每个项目会自动插入空行。空行后有非列表内容时，第二个区块会以开始项目的数字重新编号
13. 自动重新编号：按Tab再按Shift+Tab
14. 开头四个空白或Tab形成第二阶层列表
...
15. 以「星号 」(*、空白)、「减号 」或「加号 」开头
16. 列表项目间有空行时，每个项目会自动插入空行
17. 开头四个空白或Tab形成第二阶层列表
18. 项目里有「数字. 」(数字、点、空白)造成显示异常，可以在点的左侧加上跳脱字元反斜线
...
21. 用三个倒引号开头，结尾行用三个倒引号开头
22. 三个倒引号也可用波浪号（~~~）
23. 程式码每行用至少四个空白或Tab开头
